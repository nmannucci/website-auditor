...
Format your response as JSON:
{
    ...
    def _calculate_recommendation(self, audit_results: Dict) -> Dict:
        ...
        if score < 60:
            ...
            reason = "This website has significant improvement opportunities across design, conversion, and SEO. Excellent prospect."
        elif score < 75:
            recommendation = "YES"
            reason = "This website has notable areas for improvement. Good prospect for your services."
        elif score < 85:
            recommendation = "MAYBE"
            reason = "This website is decent but has some improvement opportunities. Moderate prospect."
        else:
            recommendation = "NO"
            reason = "This website is well-optimized. Limited opportunity for improvement."
...
**Report Generated By:** CPA Website Auditor

---

## 🎯 RECOMMENDATION: {rec["recommendation"]}

**Overall Score:** {rec["score"]}/{rec["max_score"]} ({rec["percentage"]}%)
...
"""
        ...
        report += f"\n---\n\n## 📞 Conversion Elements\n\n"
        conv = sections["conversion_elements"]
        report += f"- **Clear CTA:** {'✅ Yes' if conv['has_clear_cta'] else '❌ No'}\n"
        if conv["cta_details"]:
            report += f"  - {conv['cta_details']}\n"
        report += f"- **Contact Form:** {'✅ Yes' if conv['has_contact_form'] else '❌ No'}\n"
        report += f"- **Phone Number:** {'✅ Yes' if conv['has_phone_number'] else '❌ No'}\n"
        if conv["phone_numbers"]:
            report += f"  - Found: {', '.join(conv['phone_numbers'])}\n"

        report += f"\n---\n\n## 🏆 Trust Signals\n\n"
        trust = sections["trust_signals"]
        report += f"- **Team/About Section:** {'✅ Yes' if trust['has_team_info'] else '❌ No'}\n"
        report += f"- **Credentials Displayed:** {'✅ Yes' if trust['has_credentials'] else '❌ No'}\n"
        if trust["credentials_found"]:
            report += f"  - Found: {', '.join(trust['credentials_found'])}\n"
        report += f"- **Google Maps Embed:** {'✅ Yes' if trust['has_google_maps'] else '❌ No'}\n"

        report += f"\n---\n\n## 🔍 SEO Elements\n\n"
        seo = sections["seo_elements"]
        report += f"- **Meta Description:** {'✅ Yes' if seo['has_meta_description'] else '❌ No'}\n"
        if seo["meta_description"]:
            report += f"  - \"{seo['meta_description']}...\"\n"
        report += f"- **Title Tag:** {seo['title_quality']}\n"
        report += f"- **H1 Tag:** {'✅ Yes' if seo['has_h1'] else '❌ No'}\n"
        report += f"- **NAP in Footer:** {'✅ Yes' if seo['nap_in_footer'] else '❌ Incomplete'}\n"
        if seo["nap_in_footer"]:
            for key, val in seo["nap_in_footer"].items():
                report += f"  - {key}: {val}\n"

        report += f"\n---\n\n## ⚡ Technical Performance\n\n"
        tech = sections["technical"]
        report += f"- **Page Load Time:** {tech['load_time_seconds']} seconds\n"
        report += f"- **Mobile Optimized:** {'✅ Yes' if tech['has_viewport_meta'] else '❌ No'}\n"

        report += f"\n---\n\n## 📸 Screenshots\n\n"
        report += f"Desktop screenshot saved to: `{audit_results['page_data']['screenshot_path']}`\n"
        report += f"Mobile screenshot saved to: `{audit_results['page_data']['mobile_screenshot_path']}`\n"

        report += f"\n---\n\n## 💼 Next Steps\n\n"
        if rec["recommendation"] in ["STRONG YES", "YES"]:
            report += """1. **Reach out to this prospect** - they have clear needs for your services
2. Lead with the most impactful issues found above
3. Offer a free consultation to discuss improvements
4. Prepare examples of similar CPA site redesigns you've done
"""
        elif rec["recommendation"] == "MAYBE":
            report += """1. Consider reaching out if your pipeline is light
2. Focus on the specific opportunities identified
3. May require more targeted pitch around specific improvements
"""
        else:
            report += """1. This site is well-optimized - low priority prospect
2. Only reach out if you have specific premium services to offer
"""
...
def main():
    ...
    if "error" not in results:
        ...
        print(f"Score: {rec['score']}/{rec['max_score']} ({rec['percentage']}%)")
        print(f"{rec['reason']}")
        print(f"{'='*60}\n")
